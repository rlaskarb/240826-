package com.ohgiraffes.section06.finalkeyword;

public class FinalFieldTest {
    /*학습 목표 final 키워드에 대해 조금 더 자세히 이해 할 수 있따.
    *
    * 필기
    ...
    * 4. 클래수(static) 변수 : 프로그램 start 이후 값 변경 불가.
    * 5. non-static 메소드 : 메소드 재작성(overriding) 불가
    * 6. static 메소드 : 매소드 재작성(overriding) 불가
    * 7. 클래스 : 상속 불가
    *    */
    ...
    * 최기화 이후 값을 변경할 수 없기 때문에 바로 초기화를 시켜주어야 한다.
    * 그렇지 않으면 compile error 가 발생한다.
    * */

//    private final int nonStaticNum;

    /* 목차 1-1 해결 방안(1) 선언과 동시에 초기화를 한다. */
    private final int NON_STATIC_NUM =1;

    /*목차 1-2 해결방안(2) 생성자를 통해 초기화를 한다. */
...
//    private  static  final int STATIC_NUM; // 0 이라는 기본값이기 때문에 오류발생.
    ...
    * 그전에 는 초기화가 일어나지 못 한다.
    * 하지만 static 키워드는 프로그램이 run 하는 동시에 할당되기 때문에
    * 초기화가 되지 않은 상태로 선언 된것과 동일하여
