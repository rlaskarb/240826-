...
public class Application1 {
    public static void main(String[] args) {
        /*학습목표 String 과 StringBuilder 의 차이점을 알수 있다.*/
        /*필기
        * String 과 StringBuilder 의 차이
        * String
        * : 불변이라는 특성을 갖고 있다.
        *   문자열에 +연산으로 합치기를 하는경우에는
        *   기존의 인스턴스를 수정하는것이 아닌, 새로운 인스턴스를 반환한다.
        *   따라서 문자열 변경이 많이 일어나는 경우에는 성능상에 좋지 않다.
        *   하지만 반대로 변하지 않는 문자열 같은 경우에는 오히려 좋다.
        * StringBuilder
        * : 가변이라는 특성을 가지고 있다.
        *   append() 라는 메소드를 이용해서 문자열을 합치기를 할 수 있는데
        *   기존의 인ㅅ턴스를 수정할 뿐, 새로운 인스턴스를 만들 지 않는다.
